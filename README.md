...
## Решение  
Для проверки решения после поднятия стенда, выполнением команды `vagrant up` из директории `./homework`, нужно произвести записть в базу `bet`, таблицы `bookmaker`, выполнив следующую команду на сервере `master`, в результате появися запись под id `1`, со значением `1xbet`
