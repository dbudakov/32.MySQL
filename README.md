...
## Решение  
Для проверки после поднятия стенда, выполнением команды `vagrant up` из директории `./homework`, нужно произвести записть в базу `bet`, таблицу `bookmaker`, выполнив следующую команду на `master'e`, в результате появися запись под id `1`, со значением `1xbet`
